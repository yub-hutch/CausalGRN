...
#' @param group Named character vector of cell labels. Use \code{"WT"} for
#'   wild-type cells and gene names for perturbed cells.
...
simulate_grn_guided_expression <- function(
  ...
  .check_group(group, min_cells = 50L)
  if (length(group) != nrow(count)) {
    stop("'group' must have one label per row of 'count'.", call. = FALSE)
  }
  if (!identical(names(group), rownames(count))) {
    stop("'group' names must be identical to 'count' row names.", call. = FALSE)
  }
  .check_expression_simulator_params(
    d = d,
    min_coef = min_coef,
    max_coef = max_coef,
    center_normal_sd = center_normal_sd,
    center_ko_eff = center_ko_eff,
  ...
  normal_sds <- sqrt(
    rgamma(n = ngene, shape = 2, rate = 2 / (center_normal_sd ^ 2))
  )
  ...
  for (v in ordering) {
    ...
    if (length(parents) == 0) {
      ...
      wt[, v] <- rpois(
        n = nwt,
        lambda = lib_sizes[group == 'WT'] * exp(u_wt[, v])
      )
      ...
      for (ko in kos) {
        ...
        if (ko == v) {
          eff <- rbeta(
            n = npt[ko],
            shape1 = center_ko_eff * 50,
            shape2 = (1 - center_ko_eff) * 50
          )
          u_pts[[ko]][, v] <- u_pts[[ko]][, v] + log(1 - eff)
        }
        pts[[ko]][, v] <- rpois(
          n = npt[ko],
          lambda = lib_sizes[group == ko] * exp(u_pts[[ko]][, v])
        )
      ...
      for (attempt in seq_len(max_attempts)) {
        ...
        normal_means <- as.vector(
          coef[[v]][1] + u_wt[, parents, drop = FALSE] %*% coef[[v]][-1]
        )
      ...
      for (ko in kos) {
        normal_means <- as.vector(
          coef[[v]][1] + u_pts[[ko]][, parents, drop = FALSE] %*% coef[[v]][-1]
        )
  ...
  simulated_count <- do.call(rbind, c(list(wt), pts))
  simulated_group <- group[rownames(simulated_count)]
  return(list(
    dag = dag,
    coef = coef,
